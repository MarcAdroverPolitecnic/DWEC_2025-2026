<template>
  <q-page class="q-pa-md">
    ...
    <!-- Stadium Grid -->
    <div v-if="filteredStadiums.length > 0" class="row q-col-gutter-md">
      <div
          ...
          <q-card-section>
            ...
            <div class="row q-col-gutter-sm text-caption">
              ...
              <div class="col-12 text-grey-6">
                ...
                Created {{ formatDate(stadium.createdAt) }}
              </div>
            </div>
          </q-card-section>
    ...
    <q-page-sticky v-if="selectedStadium" position="bottom" :offset="[0, 18]">
      <q-banner class="bg-primary text-white shadow-2 rounded-borders">
        ...
        <span class="q-ml-sm text-caption">
          ({{ selectedStadium.totalSeats.toLocaleString() }} seats)
        </span>
...
const filteredStadiums = computed(() => {
  ...
  switch (sortBy.value) {
    ...
    case 'name-desc':
      ...
      break
    case 'seats':
      result.sort((a, b) => a.totalSeats - b.totalSeats)
      break
    case 'seats-desc':
...
function confirmDelete(stadium) {
  $q.dialog({
    ...
    stadiumStore.deleteStadium(stadium.id)
    $q.notify({
      type: 'positive',
